...
{
    // XXX Should we do our own clock routing here?
...
void rtc6705SetFreq(uint16_t mhz)
{
    if (!vtxbb.active)
        return;

    int g = mhz * RREG / (2 * Fosc);
    int nreg = g / 64;
    int areg = g % 64;
    ...
    vtxbbWrite(&vtxbb, RTC6705WDATA(1, 1, ((nreg << 7)|areg)), 25);
}

void rtc6705SetChannel(uint8_t band, uint8_t chan)
{
    rtc6705SetFreq(freqTab[band - 1][chan - 1]);
}
